resource "junos_eventoptions_destination" "testacc_v0to1_evtopts_policy" {
  lifecycle {
...
resource "junos_system_login_user" "testacc_v0to1_evtopts_policy" {
  lifecycle {
    create_before_destroy = true
  }
...
resource "junos_eventoptions_policy" "testacc_v0to1_evtopts_policy" {
  name   = "testacc_v0to1_evtopts_policy#1"
  ...
  then {
    change_configuration {
      ...
      retry_count                = 2
      retry_interval             = 1
      user_name                  = junos_system_login_user.testacc_v0to1_evtopts_policy.name
    }
    event_script {
      filename = "filename_1"
      arguments {
      ...
      destination {
        name           = junos_eventoptions_destination.testacc_v0to1_evtopts_policy.name
        retry_count    = 1
        retry_interval = 2
        transfer_delay = 3
      }
    ...
    upload {
      filename    = "filename_5"
      destination = junos_eventoptions_destination.testacc_v0to1_evtopts_policy.name
    }
    upload {
      filename       = "filename_4"
      destination    = junos_eventoptions_destination.testacc_v0to1_evtopts_policy.name
