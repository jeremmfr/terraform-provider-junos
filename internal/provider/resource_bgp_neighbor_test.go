...
func TestAccResourceBgpNeighbor_writeOnly(t *testing.T) {
	if os.Getenv("TESTACC_SWITCH") == "" {
		resource.Test(t, resource.TestCase{
			PreCheck:                 func() { testAccPreCheck(t) },
			ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
			...
			Steps: []resource.TestStep{
				{
					ConfigDirectory: config.TestStepDirectory(),
					Check: resource.ComposeTestCheckFunc(
						resource.TestCheckNoResourceAttr("junos_bgp_neighbor.testacc_bgpneighbor_wo",
							"authentication_key"),
						resource.TestCheckNoResourceAttr("junos_bgp_neighbor.testacc_bgpneighbor_wo",
							"authentication_key_wo"),
						resource.TestCheckResourceAttr("junos_bgp_neighbor.testacc_bgpneighbor_wo",
