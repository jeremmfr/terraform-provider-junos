...
* **Terraform 1.0 or later is now required**: the provider is now served with version 6 of the Terraform plugin protocol instead of version 5, to be able to use nested attributes in resource schemas, so the registry won't offer new releases of the provider to an older Terraform CLI
...
* **resource/junos_eventoptions_destination**: add `password_wo` write-only argument in `archive_site` block and its `password_wo_version` companion, to be able to use an ephemeral value for the archive site password without storing it in the Terraform state (Partial fix [#914](https://github.com/jeremmfr/terraform-provider-junos/issues/914))
...
* **resource/junos_interface_logical**: add `authentication_key_wo` write-only argument in `vrrp_group` block in `address` block in `family_inet` block and its `authentication_key_wo_version` companion, to be able to use an ephemeral value for the authentication key without storing it in the Terraform state (Partial fix [#914](https://github.com/jeremmfr/terraform-provider-junos/issues/914))
...
* **resource/junos_security_authentication_key_chain**: add `key_secret_wo` write-only argument, a map of block keyed by the `id` of the `key` block with a `value` and a `version` attribute, to be able to use ephemeral values for the authentication keys without storing them in the Terraform state, `secret` argument in `key` block is now optional and one of `secret` or an entry with the same `id` in `key_secret_wo` is required (Partial fix [#914](https://github.com/jeremmfr/terraform-provider-junos/issues/914))
...
* **resource/junos_services_user_identification_ad_access_domain**: add `user_password_wo` write-only argument and `user_password_wo` write-only argument in `user_group_mapping_ldap` block, with their `*_wo_version` companions, to be able to use ephemeral values for the passwords without storing them in the Terraform state, `user_password` argument is now optional and one of `user_password` or `user_password_wo` is required (Partial fix [#914](https://github.com/jeremmfr/terraform-provider-junos/issues/914))
...
* **resource/junos_system**: add `secret_wo` and `preauthentication_secret_wo` write-only arguments in `destination_radius_server` block in `accounting` block, `secret_wo` write-only argument in `destination_tacplus_server` block in `accounting` block, `password_wo` write-only argument in `archive_site` block in `archival_configuration` block and `autoupdate_password_wo` write-only argument in `license` block, with their `*_wo_version` companions, to be able to use ephemeral values without storing them in the Terraform state, `secret` argument in `destination_radius_server` block is now optional and one of `secret` or `secret_wo` is required (Partial fix [#914](https://github.com/jeremmfr/terraform-provider-junos/issues/914))
...
* **resource/junos_system_syslog_file**: add `password_wo` write-only argument in `sites` block in `archive` block and its `password_wo_version` companion, to be able to use an ephemeral value for the archive site password without storing it in the Terraform state (Partial fix [#914](https://github.com/jeremmfr/terraform-provider-junos/issues/914))
